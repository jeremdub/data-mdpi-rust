#!/usr/bin/env python3
"""E4: cross-stack interoperability, dot15d4 against Contiki-NG (Table 8 of the
paper).

One coordinator and one leaf, one stack each, at the 1 us rtimer tick of E3c.
Both roles are tested: a dot15d4 coordinator with a Contiki-NG node (variant A)
and the reverse (variant B). The joined node sends one acknowledged data frame
every 15 s, which is what carries the Time Correction IE and exposed the
48 us timing reference error of the Contiki-NG nRF port. Variant B was run
before and after that fix.

The two stacks must agree on the PAN identifier and the channel: 0xabcd and
channel 26 alone. A dot15d4 leaf takes SCHEDULE=learn, so it copies the
slotframe and the links from the first Enhanced Beacon it hears.

    ./experiment.py --coordinator contiki --leaf rust    --run 7200  # variant B
    ./experiment.py --coordinator rust    --leaf contiki --run 7200  # variant A

Contiki-NG in both roles is the baseline of E3b and E3c, which
`e3b-e3c-sync_error_contiki/experiment.py` runs instead, without data frames.

Needs CONTIKI in the environment, pointing at the patched Contiki-NG tree.
`check_e4.py` recomputes Table 8 from the saved logs of the three runs.
"""
...
# and serial port. ROOT_ADDR is the coordinator board's FICR extended address,
# the 16-hex value it prints at boot as `node N: PARENT=<hex>`. It is only
# needed when the leaf uses a static schedule.
...
# Both stacks must agree on these, or the leaf never hears the coordinator.
# 0xabcd is the Contiki-NG default; the Rust default (0xbeef) is overridden
# below. The Rust stack scans and hops on channel 26 alone, so Contiki-NG needs
# a single-channel hopping sequence {26} as well.
PAN_ID = "0xabcd"
CHANNEL = 26
...
# Cell layout of the Rust nodes (see src/bin/tsch/README.md). "learn" makes the
...
def leaf_application(stack: str):
    ...
    if stack == "contiki":
        return contiki.Application(app_folder=CONTIKI_APP, target="joining-node", environment_variables={"SENDER":"1"})
    env = {
        "NODE_ID": "1",
        ...
        "DOT15D4_MAC_PAN_ID": PAN_ID,
        "SENDER": "1",
        # One acknowledged data frame every 15 s, as in the runs of Table 8.
        "SEND_PERIOD_MS": "15000",
...
def build_experiment(coordinator: str, leaf: str, run_s: float) -> tuple[Experiment, list]:
...
def main() -> None:
    ...
    env = Local(
        ...
        # ioteapot captures the pcap itself; do not also run Wireshark on the
        # same sniffer dongle.
