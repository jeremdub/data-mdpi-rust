#!/usr/bin/env python3
"""E2: single-hop synchronization accuracy, measured twice (Table 6 of the
paper).

A star of one root (NODE_ID 0) and ten leaves on a 101-slot slotframe, with a
10 s beacon period. Every leaf logs its own synchronization error, and one
probed leaf plus the root are wired to a Saleae Logic Pro 16 through
`radio-trace`, so the same beacons are measured by the stack and by the
analyzer. The paired series qualify the in-stack log as the instrument of the
multi-node results. The first error a leaf logs also gives its crystal offset
from the root.

Boards are within radio range of each other, so the star is logical: the
schedule decides who is audible and PARENT pins the time source.
...
root's FICR address), then:
    python experiment.py --run 21600 --variant simultaneous   # as in the paper
    python experiment.py                                      # 15 min, to check the bench

Writes e2_<variant>_sync.csv and trace.pcap. Export the analyzer capture as
logic-analyzer.csv next to the log; `check_e2.py` pairs the two and recomputes
Table 6.
"""
